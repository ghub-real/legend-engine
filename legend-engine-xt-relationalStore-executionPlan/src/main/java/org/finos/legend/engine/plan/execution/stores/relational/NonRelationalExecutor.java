...
import org.finos.legend.engine.plan.execution.stores.relational.result.NoSQLExecutionResult;
import org.finos.legend.engine.plan.execution.stores.relational.tempdocumentconnection.TempLocalMongoDbClient;
...
//    }
    ...
    {
        this(temporarytestdb, nonRelationalExecutionConfiguration, Optional.empty());
    }

    public NonRelationalExecutor(TemporaryTestDbConfiguration temporarytestdb, NonRelationalExecutionConfiguration nonRelationalExecutionConfiguration, Optional<DatabaseAuthenticationFlowProvider> flowProviderHolder)
    {
        this.flowProviderHolder = flowProviderHolder;
        this.connectionManager = new ConnectionManagerSelector(temporarytestdb, nonRelationalExecutionConfiguration.oauthProfiles, flowProviderHolder);
...
//        this.resultInterpreterExtensions = Iterate.addAllTo(ServiceLoader.load(ResultInterpreterExtension.class), Lists.mutable.empty()).collect(ResultInterpreterExtension::additionalResultBuilder);
    ...
    public Result execute(MongoQLExecutionNode node, MutableList<CommonProfile> profiles, ExecutionState executionState)
    {
        ...
        NonRelationalDatabaseConnection databaseConnection = (NonRelationalDatabaseConnection) node.connection;
       ...
       //DatabaseManager databaseManager = DatabaseManager.fromString(databaseType);
        ...
        TempLocalMongoDbClient mongoClient = null;
        String query = node.getQuery();

        if (!Objects.equals(query, "") && query != null) {
            try {
                mongoClient = new TempLocalMongoDbClient(datasourceSpecification);
                results = mongoClient.executeCustomAggregationQueryToDefaultDB(query);
            } finally {
                if (mongoClient != null) {
                    mongoClient.close();
                }
...
//        }
        ...
        return new NoSQLExecutionResult(executionState.activities, node, databaseType, databaseTimeZone, profiles, tempTableList, executionState.topSpan);
    }
