...
import org.finos.legend.engine.plan.execution.stores.relational.result.BiFunctionHelper;
...
public class ResultField
{
    ...
    ResultField(int fieldIndex, String label, String dataType, int dbMetaDataType)
    {
    ...
    private void createValueExtractors()
    {
        ...
        if ((this.dbMetaDataType == Types.DATE) || "DATE".equalsIgnoreCase(this.dataType))
        {
            this.valueExtractor = BiFunctionHelper.unchecked(
                    (resultSet, calendar) -> resultSet.getDate(this.fieldIndex)
            );
        }
        else if ((this.dbMetaDataType == Types.TIMESTAMP) || "TIMESTAMP".equalsIgnoreCase(this.dataType))
        {
            this.valueExtractor = BiFunctionHelper.unchecked(
                    (resultSet, calendar) -> resultSet.getTimestamp(this.fieldIndex, calendar)
            );
        }
        else
        {
            this.valueExtractor = BiFunctionHelper.unchecked(
                    (resultSet, calendar) -> resultSet.getObject(this.fieldIndex)
            );
        }
        ...
        switch (this.dbMetaDataType)
        {
            case Types.DATE:
            {
                this.transformedValueExtractor = BiFunctionHelper.unchecked(
                        (resultSet, calendar) ->
                        {
                            java.sql.Date date = resultSet.getDate(this.fieldIndex);
            ...
            case Types.TIMESTAMP:
            {
                this.transformedValueExtractor = BiFunctionHelper.unchecked(
                        (resultSet, calendar) ->
                        {
                            java.sql.Timestamp timestamp = resultSet.getTimestamp(this.fieldIndex, calendar);
            ...
            case Types.BIGINT:
            {
                this.transformedValueExtractor = BiFunctionHelper.unchecked(
                        (resultSet, calendar) ->
                        {
                            long num = resultSet.getLong(this.fieldIndex);
            ...
            case Types.DOUBLE:
            {
                this.transformedValueExtractor = BiFunctionHelper.unchecked(
                        (resultSet, calendar) ->
                        {
                            double num = resultSet.getDouble(this.fieldIndex);
                            return !resultSet.wasNull() ? num : null;
                        }
                );
                break;
            ...
            {
                this.transformedValueExtractor = BiFunctionHelper.unchecked(
                        (resultSet, calendar) -> resultSet.getBigDecimal(this.fieldIndex)
                );
                break;
            }
            case Types.CHAR:
            ...
            {
                this.transformedValueExtractor = BiFunctionHelper.unchecked(
                        (resultSet, calendar) -> resultSet.getString(this.fieldIndex)
                );
                break;
            }
            case Types.BIT:
            case Types.BOOLEAN:
            {
                this.transformedValueExtractor = BiFunctionHelper.unchecked(
                        (resultSet, calendar) ->
                        {
                            boolean bool = resultSet.getBoolean(this.fieldIndex);
            ...
            case Types.LONGVARBINARY:
            {
                this.transformedValueExtractor = BiFunctionHelper.unchecked(
                        (resultSet, calendar) ->
                        {
                            byte[] bytes = resultSet.getBytes(this.fieldIndex);
