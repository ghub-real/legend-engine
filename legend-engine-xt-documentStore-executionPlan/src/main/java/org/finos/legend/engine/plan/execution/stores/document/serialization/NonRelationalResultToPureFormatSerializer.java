...
package org.finos.legend.engine.plan.execution.stores.document.serialization;
...
import org.finos.legend.engine.plan.execution.stores.document.result.NonRelationalResult;
...
public class NonRelationalResultToPureFormatSerializer extends Serializer
{
    ...
    protected NonRelationalResult nonRelationalResult;

    public NonRelationalResultToPureFormatSerializer(NonRelationalResult nonRelationalResult, byte[] start_token, byte[] end_token)
    {
        this.nonRelationalResult = nonRelationalResult;
    ...
    public void stream(OutputStream stream)
    {
        ...
        {
            this.nonRelationalResult.close();
        }
    }
    ...
    {
        this.processColumns(outputStream, ((TDSBuilder) nonRelationalResult.builder).columns);
    }

    public void streamRows(OutputStream outputStream) throws Exception
    {
        int rowCount = 0;
        try (Scope ignored = GlobalTracer.get().buildSpan("Non Relational Streaming: Fetch first row").startActive(true))
        {
//            if (!nonRelationalResult.resultSet.isClosed() && nonRelationalResult.resultSet.next())
//            {
//                this.processRow(outputStream);
//                rowCount++;
//            }
        }
        try (Scope scope = GlobalTracer.get().buildSpan("Non Relational Streaming: remaining rows").startActive(true))
        {
//            while (!nonRelationalResult.resultSet.isClosed() && nonRelationalResult.resultSet.next())
//            {
...
//            }
            ...
            if (nonRelationalResult.topSpan != null)
            {
                nonRelationalResult.topSpan.setTag("lastQueryRowCount", rowCount);
            }
        }
    }
}
