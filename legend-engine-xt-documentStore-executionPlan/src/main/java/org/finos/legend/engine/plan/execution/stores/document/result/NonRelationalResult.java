...
//            }
    ...
    public NonRelationalResult(DocumentQueryExecutionResult documentQueryExecutionResult, RelationalInstantiationExecutionNode node)
    {
        ...
        this.databaseTimeZone = documentQueryExecutionResult.getDatabaseTimeZone();
        //this.temporaryTables = documentQueryExecutionResult;
        this.topSpan = documentQueryExecutionResult.getTopSpan();

        try
        {
            //this.connection = documentQueryExecutionResult.getConnection();
            //this.statement = connection.createStatement();
            //this.resultSet = documentQueryExecutionResult.();
            this.executedMongoQL = documentQueryExecutionResult.getExecutedMql();
            //this.resultSetMetaData = documentQueryExecutionResult.getResultSetMetaData();
    ...
    private void buildTransformersAndBuilder(ExecutionNode node, DatabaseConnection databaseConnection) throws SQLException
    {
        ...
        if (ExecutionNodeTDSResultHelper.isResultTDS(node))
        {
            ...
            for (int columnIndex = 1; columnIndex <= this.fieldCount; columnIndex++)
            {
            ...
            this.builder = new TDSBuilder(node, this.noSQLFields, isDatabaseIdentifiersCaseSensitive);
            this.fieldListForSerializer = ListIterate.collect(((TDSBuilder) this.builder).columns, col -> col.name);
        }
    ...
    public Object getTransformedValue(int columnIndex) throws SQLException
    {
        ...
        switch (resultSetMetaData.getColumnType(columnIndex))
        {
            ...
            {
                Timestamp timestamp = this.resultSet.getTimestamp(
                        columnIndex,
